    ...
    function mint_configLoadedAccount() {
        GM_addStyle(`#characterBin {
          ...
          #characterBin[open]::before {
            ...
            if(mint_binnedChars.length === 0){
                localStorage.setItem('mint_binnedChars', JSON.stringify("{}"));
            }
          ...
          var charList = document.getElementsByClassName("form-group")[2];
          var labelElement = charList.getElementsByTagName("label")[0];
          var appendLoc = labelElement.nextSibling;
      ...
      <summary>
        <div class="btn btn-default btn-block" style="border-top-right-radius: 0; border-bottom-right-radius: 0;">
          ...
          charList.appendChild(mint_characterBin);
    ...
    var editList = currentElement.querySelector('ul'); 
          ...
          document.addEventListener('click', function(event) {
            ...
            if (clickedElement.classList.contains('remove-from-bin')) {
                handleRemoveFromBin(editLi);
            } else if (clickedElement.classList.contains('send-to-bin')) {
                handleSendToBin(editLi);
            }
        });
