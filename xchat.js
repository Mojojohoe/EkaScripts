...
(function($) {
    ...
    ChatPane.prototype.setbg = function setBackgroundImage(url) {
        this.$chatPane.css({
            backgroundImage: url ? ('linear-gradient(to bottom, rgba(0,0,0,0.2), rgba(0,0,0,0.9)), url("' + url + '")') : ''
        });
    };
...
(function($) {
    ...
    function HoverBoxSystem(leash) {
        if (typeof leash === "object") {
            ...
            return wWindows[leash] || new HoverBox({
                leash: leash
            });
        }
    }
  ...
  mint_filters.forEach(function(filter) {
    ...
    if (filter.names && filter.names.includes(ule.charName)) {
      ...
      if (caseValue === 0) {
        ...
        if (behave1 === 3 || behave2 === 3  && !$ule.is('.highlighted')) {
          $ule.find('.highlight').trigger('click');	
        }

        if (behave1 === 4 || behave2 === 4 && !$ule.is('.ignored')) {
      ...
      if (caseValue === 1 && trigger === 1) {
        ...
        if (!iconSpan.length || iconSpan.hasClass('status-online')) {
          ...
          if (behave1 === 3 && !$ule.is('.highlighted')) {
	   $ule.find('.highlight').trigger('click');	  

          }

          if (behave1 === 4 && !$ule.is('.ignored')) {
            $ule.find('.ignore').trigger('click');	  
          }
        }
      }
...
function applyRoomEntryData(data) {
    ...
    data.motd = data.motd + "<span style='color:#ccffff'>You're running <span style='color:#54f0be'>mint <span style='color:#ccffff'>" + mint_version + "</span><br> Thank you for helping test mint, " + me.name + ". Please give your feedback to Jobix.</span></span>"	 
...
function ctxOpenHoverboxForHoverbox() {
    ...
    tab.exists() ? tab.show() : tab.create({
        leash: msg.from.name,
        type: "whisper"
    }).show();
}
