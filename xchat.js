...
﻿(function($,len,createRange,duplicate){
	$.fn.caret=function(options,opt2){
		var start,end,t=this[0],oldBrowser=(window.getSelection ? false : true);
        ...
        this.filter('textarea').each(function(){
            ...
            var textarea = $(this).css({resize:'none','overflow-y':'hidden'}),
                ...
                updateSize = function() {
                    ...
                    // Prepare the clone:
                    clone.height(0).val($(this).val()).css("width", textarea.css("width")).scrollTop(10000);
...
(function($)
{
    ...
    CompletionState.prototype.complete = function complete()
    {
        ...
        if (!(this.tabInProgress))
        {
            ...
            else if (this.lastSpace >= (caret.end-1) || ++(this.lastSpace) >= text.length) {
        ...
        if (this.lastSpace >= 1) {
            this.el.value = text.substring(0, this.lastSpace) + replacement + this.afterBase;
            $(this.el).caret((replacement.length + this.lastSpace), (replacement.length + this.lastSpace));
        }
        else {
...
(function($) {
    ...
    var ChatPane = function(element, options) {
        ...
        if (this.options.pauseButton) {
            ...
            this.$chatPane.on("scroll.rya.chatpane", function() {
                thisChatPane.toggle((this.scrollTop + $(this).height() - this.scrollHeight) < -40);
            });
        } else {
    ...
    ChatPane.prototype.setbg = function setBackgroundImage(url) {
        this.$chatPane.css({backgroundImage: url ? ('linear-gradient(to bottom, rgba(0,0,0,0.2), rgba(0,0,0,0.9)), url("' + url + '")') : '' });
    };
    ...
    function Plugin(command) {
        ...
        return this.each(function () {
            ...
            if (!data) $this.data('rya.chatpane', (data = new ChatPane(this, $this.data())))
...
var COMMAND_HANDLERS = {
    ...
    invite: function handleInvite(evt) {
        standardAjaxAction({
            data: {action: "inviteToGroupChat", groupId: evt.$msgTargetCtl.val(), charName: evt.args[0]}
        }).fail(function(error, message) {
            $.jGrowl(error + "\n" + message, {sticky: true, header: "Error Inviting To Group"});
        }).done(function() {
            evt.$bodyCtl.val("");
        });
        return false;
    ...
    room: function handleLook(evt) {
        ...
        standardAjaxAction({
            data: {action: "changeRoom", roomId: evt.args[0]}
        });
        evt.$bodyCtl.val("");
        return false;
    },
    ...
    pub: function handlePub(evt) {
        ...
        wrapXhrForStandard($.post("pubAction.srv", {
            action: "sendPub",
    ...
    broadcast: function handleBroadcast(evt) {
        ...
        wrapXhrForStandard($.post("modAction.srv", {
    ...
    who: function handleWho(evt) {
        ...
        wrapXhrForStandard($.get("info.srv", {
            dataset: "charinfo",
            charName: evt.args[0]
        })).done(function(charData) {
            $.jGrowl(charData.accountName, {header: "Account of " + charData.charName});
            evt.$bodyCtl.val("");
        });
        return false;
    },
...
function handleListAndOnline(evt, onlineOnly) {
    wrapXhrForStandard($.get("adminInfo.srv", {
        ...
        $.each(charList, function() {
            $("<li>").append(
                    $("<a target='_blank'>").text(this.charName)
                    .attr("href", "../../profile/" + this.charName)).appendTo($ul);
        });
        $.jGrowl($ul, {header: (onlineOnly ? "Online " : "") + "Characters of " + evt.args[0]});
        evt.$bodyCtl.val("");
    });
    return false;
}
...
(function($) {
    ...
    function HoverBoxSystem(leash) {
        if (typeof leash === "object") {
            ...
            return wWindows[leash] || new HoverBox({leash: leash});
        }
    }
    ...
    HoverBoxSystem.saveOpenTabs = function saveOpenTabs(prefix) {
        if ('sessionStorage' in window) {
            sessionStorage.setItem(prefix + ".savedHoverBoxes", JSON.stringify(HoverBoxSystem._serializeOpenTabs()));
        }
    };
    ...
    HoverBox.prototype.create = function createChatTab(options)
    {
        if (!this.exists())
        {
            ...
            this.$tabContent = $($.parseHTML(Mustache.render(tabContentTemplate, this)));
            ...
            if (!e.isDefaultPrevented()) {
                ...
                mainWindow.$chatPane.children("[leash='" + this.leash + "']").appendTo(this.$chatPane);
                this.$chatPane.chatpane("scroll");
                this.$tabContent.find(".sender-body").tabcomplete({matchFunction: tabCompleteMatch});
            }
    ...
    HoverBox.prototype.appendToChatPane = function appendToChatPane($msg)
    {
        ...
        if (window.isFocused === false && this !== mainWindow 
    ...
    HoverBox.prototype.close = function closeTab()
    {
        ...
        this.$tab.trigger(e);
        if (!e.isDefaultPrevented()) {
            mergeMessagesIntoChat(mainWindow.$chatPane, this.$chatPane.children(".chatmsg").remove().toArray());
        }
        
        // Close the tab
        var $cleanup = $([this.$tab.get(0), this.$tabContent.get(0)]);
        if (this.$tab.hasClass("active")) {
            $("#main-tab").one("shown.bs.tab", function() {
                $cleanup.remove();
            }).tab("show");
        } else {
            $cleanup.remove();
    ...
    window.initChatTabs = function initChatTabs()
    {
        ...
        $("#tabs-pane").on("show.bs.tab.hbox", "#main-tab", function(event) {
            $collapser.slideUp(150);
        }).on("shown.bs.tab.hbox", function(event) {
            if (event.target.id !== "main-tab") {
                if (!$collapser.get(0).style.height) $collapser.height($("#chat-pane").height() / 2);  // Default height to half
...
function handleChatMessage(msg)
{
    if (ses.ignored[msg.from.id])
        return; // Bail out early if we're ignoring them.
...
function handleChangeRoom(msg)
{
    ...
    standardAjaxInfo("initialchat-room").done(function(result) {
        ...
        // Disable current room on room list
         $("#room-list [data-code='" + ses.roomId + "']").parent().siblings().removeClass("disabled").end().addClass("disabled");
...
function handleMinimapPinUpdate(pin) {
    ...
    if ($pin.length === 0) $pin = $("<div class='pin'>")
            .attr("data-cid", pin.charId)
            .attr("title", pin.charName).appendTo("#minimap-container");
    $pin.attr("title", pin.charName).css({
...
function setWeather(newState, transitionMessage, newImage, newIcon) {
    ...
    appendToChatPane($chatPane, $("<p>" + transitionMessage + "</p>").attr("title", newState));
}
...
function handleGroupchatJoined(msg) {
    ...
    if ($("#groupchat-list .groupchat[data-pmtarget='" + msg.groupchat.leash + "']").length === 0) {
...
function handleGroupchatLeft(msg) {
    if (msg.leaver.id === me.charId) {
        ...
        $("#groupchat-list .groupchat[data-pmtarget='" + msg.groupchat.leash + "']").closest("li").andSelf().remove();
    }
}
...
function handlePragma(msg) {
    try {
        ...
        $.jGrowl("" + ex, {sticky: true, header: "Unexpected Error"});
...
var $chatPane, lastMsgId = 0, selectedColor = 1;
...
function onDocumentReady()
{
    ...
    $("#main-sender-body").autoResize({extraSpace: 0, limit: 300, onResize: layoutEntirePage});
...
function updateSettings(settings) {
    ...
    // Hide/Show Eyecons
    $("body")[me.settings.hideEyecons ? 'addClass' : 'removeClass']("hide-eyecons");
}
...
{
    return standardAjaxAction({data: $.extend({action: action}, data)});
}
...
function standardAjaxAction(options) {
    $("body").addClass("busy");
    var flan = $.extend(true, {url: "action.srv", type: "post", data: {csrf: me.csrf}}, options);
    return wrapXhrForStandard($.ajax(flan)).fail(function(error, message) {
...
function tabCompleteMatch(text, lastWord)
{
    ...
    // Otherwise get list of matching names
        ...
        $("#ulist-itself [data-pmtarget]").each(function() {
            ...
            if (name && name.toLowerCase().indexOf(lastWord) === 0 && retVal.indexOf(name) < 0) {
                retVal.push(name);
            }
        });
    }
 ...
 */
var FORMATTING_SHORTCUT_KEYS = {73: "i", 85: "u", 66: "b" }; 
...
function formattingReplace($thing, letter)
{
    ...
    if (caret.start !== caret.end) {
        ...
        $thing.caret({start: caret.start, end: caret.end + 7});
    }
    else {
        ...
        $thing.val(caret.replace(lastOpen > lastClose ? "[/" + letter + "]" :"[" + letter + "]"));
        var newPos = caret.start + (lastOpen > lastClose ? 4 : 3);
        $thing.caret({start: newPos, end: newPos});
    }
}
...
function sendMessage(form, $bodyCtl, $msgTargetCtl)
{
    ...
    wrapXhrForStandard($.ajax({
        url: "action.srv",
        type: "post",
        data: {action: "post", msgBody: body, msgTarget: toName, color: selectedColor, csrf: me.csrf}
    })).fail(function(error, message) {
...
function showChatErrorMessage(error, message, time) {
    var $alert = $('<div role="alert" class="alert alert-warning alert-dismissible fade in">' +
            '<button aria-label="Close" data-dismiss="alert" class="close" type="button">' +
            '<span aria-hidden="true">&times;</span></button></div>');
    $alert.append(error + (!!message === false || message === error ? "" : "\n" + message));
    setTimeout(function () {
        $alert.alert("close");
    }, (time > 0 ? time : 4000));
...
function setupMessageFetch()
{
    ...
    function fetchMessages()
    {
        ...
        return $.ajax({
            ...
            if (data && data.error && data.error === "NotLoggedIn") {
                ...
                alert("Warning: An impossible error happened. Inform Leshana the server sent: " + JSON.stringify(data));
            }
            else {
    ...
    function onFetchError(jqXHR, textStatus, errorThrown) {
        ...
        var $warnMsg = $("<p class='error bg-warning'>Problem connecting to server: " + errorThrown + " (" + textStatus + ")</p>");
        appendToChatPane($chatPane, $warnMsg);
        
        // Check whether to give up or keep trying
        if (window.errorCount > 10) {
            appendToChatPane($chatPane, $("<p class='error bg-danger'>Giving up. (Too many failures)</p>"));
            return;
        }
        else {
            var waitSecs = (window.errorCount - 1) * 2;
            $warnMsg.append($("<br><span>Trying again in " + waitSecs + " seconds</span>"));
            setTimeout(function() {
                fetchMessages().then(function() {
                    $warnMsg.after($("<p class='error bg-success'>Reconnection successful!<p>"));
                    window.errorCount = 0;
                });
            }, waitSecs*1000);
        }
    ...
    function onNotLoggedIn(data) {
        $chatPane.chatpane("append",  $("<p class='error bg-warning'>" + data.message + "</p>"));
    }
...
function loadInitialChat()
{
    ...
    standardAjaxInfo("initialchat-all").done(function(result)
    {
        ...
        $chatPane.chatpane("append", $("<div class='alert alert-danger'>Failed to connect to chat server.</div>"));
    });
}
...
function applyRoomEntryData(data)
{
    ...
    mergeMessagesIntoChat($chatPane, jQuery.map(data.initialChat, function(msg) {
...
function renderMotd(motd) {
    appendToChatPane($("#chat-pane"), $("<div class='motd'>")
            .attr("id", "srt" + lastMsgId)
            .attr("leash", "ROOM:" + ses.roomId).html(motd));
}
...
function renderRoomList(data) {
    ...
    var template = '{{#.}}<li><a href="#" data-code="{{id}}">{{{prettyName}}} ({{userCount}})</a></li>{{/.}}';
...
function onClickRoomList(event) {
    ...
    chatAction("changeRoom", {roomId: newRoom});
}
...
function renderFavColors(data) {
    var FAVCOLOR_TEMPLATE = '{{#.}}<input type="radio" name="color" id="fc{{id}}" value="{{id}}">'
...
function onClickFavColor() {
    ...
    $("#sender-body-color").html(".sender-body{color:" + this.style.backgroundColor + ";}");
    // Save in local storage.
    if ('sessionStorage' in window) sessionStorage.setItem(me.name + ".selectedColor", window.selectedColor);
}
...
function renderStatusList(data) {
    ...
    var template = '{{#.}}<li><a href="#" data-code="{{code}}"><i class="icon status-{{code}}"></i> {{{name}}}</a></li>{{/.}}';
    var rendered = Mustache.render(template, data);
    $(rendered).appendTo($statusList);
}
function onClickStatusList(event) {
    var newStatus = $(this).attr("data-code");
    chatAction("setStatus", {newStatus: newStatus}).done(function() {
...
function renderGroupchatList(data) {
    var template = '{{#.}}<li><a href="#" class="pmclick groupchat" data-pmtarget="{{leash}}" data-open="groupchat" title="{{name}}"><i class="glyphicons glyphicons-conversation"></i> {{{prettyName}}}</a></li>{{/.}}';
...
function onClickHighlight(event) {
    ...
    chatAction("highlightCharacter", {charId: cid, newVal: ses.highlighted[cid]});
    event.preventDefault();
}
...
function onClickMinimap(e) {
    var rect = document.getElementById("minimap-container").getBoundingClientRect();
    var offsetX = e.clientX - rect.left;
    var offsetY = e.clientY - rect.top;
    chatAction("post", {msgBody: ("/minimap "  + offsetX + " " + offsetY)});
}
...
function onClickGroupChatShowMembers(event) {
    ...
    standardAjaxInfo("groupuserlist", {data:{leash: leash}}).then(function(result) {
        var $list = $($(event.target).data("target")).children("ul");
        $list.empty();
        $.each(result, function() {
            $list.append($("<li><a href='#' data-pmtarget='" + this.charName + "'>" + this.prettyName + "</a></li>"));
        });
    });
}
...
function onClickHoverBoxLoadHistory(event) {
    ...
    var msgId = $flan.length > 0 ? $flan.closest(".chatmsg").attr('id').substring(3) : 0;
    var data = {
        leash: $this.data().leash,
        before: msgId
    };
    $this.addClass("loading")
    standardAjaxInfo("scrollback", {data:data}).then(function(results) {
        if (results.length === 0) {
            $this.parent().remove();
...
function buildObjectContextMenu($trigger, e)
{
    ...
    var items = {};
    if (isCharacter) items.profile = {name: "Open Profile", callback: ctxOpenProfile};
    items.opentab = {name: "Open Hoverbox", callback: pmTargetFunction};
    if (isCharacter) items.invite = {name: "Invite to Groupchat", callback: ctxInviteToGroupchat, items: buildGroupchatListSubMenu()};
    if (isCharacter) items.sendpub = {name: "Send PUB", callback: ctxSendPub};
    return {items: items};
}
...
function ctxOpenHoverboxForHoverbox() {
    ...
    tab.exists() ? tab.show() : tab.create({leash: msg.from.name, type: "whisper"}).show();
}
...
function ctxOpenHoverbox() {
    ...
    tab.exists() ? tab.show() : tab.create({type: type, leash: opts.pmtarget, title: opts.pmtitle}).show();
}
...
function ctxInviteToGroupchat(gid) {
   ...
   chatAction("inviteToGroupChat", {charName: target, groupId: gid});
}
...
function onClickLeaveGroupChat(event) {
    event.preventDefault();
    var leash = $(this).closest("[data-leash]").data("leash");
    chatAction("leaveGroupChat", {groupId: leash}).then(function() {
...
function attachEventHandlers()
{
    ...
    // "My Summary"
    $("#editSummaryModal").on("show.bs.modal", function() {
        standardAjaxInfo("charinfo", {data: {charName: me.name}}).then(function(result) {
    ...
    // Admin Help
    $("#adminHelpModal").on("shown.bs.modal", function() {
        $("#reportBody").focus();
    }).on("submit", "form", function(event) {
        event.preventDefault();
        wrapXhrForStandard($(this).ajaxSubmit().data('jqxhr')).then(function() {
    ...
    // Make pressing enter in textarea submit the form.
    $("#main-sender-form, #hoverbox-pane").on("keydown", "textarea", onEnterPressedInTextarea);
    
    // Keyboard shortcuts for formatting - on all textareas.
    $("#main-sender-form, #hoverbox-pane").on("keydown", "[name=body]", onKeydownBodyFormatting);
    
    // Tab Completion
    $("#main-sender-body").tabcomplete({matchFunction: tabCompleteMatch});
    ...
    $(document).on("click", ".msg-delete", function() {
        ...
        if (confirm("Are you sure you want to delete the message from " + $(".name", this.parentNode).text() + "?")) {
            chatAction("deleteMessage", {msgId: msgId});
        }
...
function onClickLogoutLink(event) {
    ...
    if ($this.data('WaitForConfirm') === true) {
        ...
        setTimeout(function() { $this.data('WaitForConfirm', false).children("span").text("Logout"); }, 2000);
    }
    event.preventDefault();
